...
- **The cap is therefore spent on ordinary runs, and the bound is conditional:** 5,000 rows, unless the player holds more records than that, in which case the records win and the log is exactly their records. A cap may cost a player their history; it may not cost them their bests. The first version of the fold tried to have it both ways — it preserved records by putting them back at the *front* of the log, which is inside the window the next fold re-examines, so they were preserved again and never consumed. The log settled at 5,000 *plus every record ever earned* and grew for as long as the player kept improving. Nothing was lost, but a cap that is not a cap eventually meets the quota error below, which is swallowed on purpose.
