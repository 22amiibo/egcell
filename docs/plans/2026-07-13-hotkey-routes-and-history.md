...
### 1a.12 Phase 4 addendum: what the solver found, and what it cost

Phase 4's acceptance test — *every generated variant and every classic yields a route* — failed on **21 of 128** on its first run. Everything below came out of that, and two of the items are corrections to this plan rather than to the code.

**§6.3's core assumption is wrong: "navigate to a cell holding West, then filter" does not fall out of the search for free.** The plan reasons that because `FILTER_TO_VALUE` filters by the active cell's value, "filter to West" is just *navigate to a cell holding West, then filter*, and the BFS discovers it. True in principle, false at the stated budget: a difficulty-5 table is over twenty rows tall, so a cell in the middle of one is over twenty arrow presses from the corner, and `maxDepth: 8` cannot reach it. Every drill whose target was a *specific row* — `find-name`, `first-blank`, both filters, `row-by-name`, `column-values` — came back null. The defaults become **`maxDepth: 32`, `maxNodes: 1_000_000`**. Depth alone would have made it worse (a deeper frontier multiplies range selections faster than it finds routes); it is affordable only alongside the prunes below.

**The solver was searching actions the game refuses — a correctness bug, found while chasing the budget.** `SpreadsheetGrid` gates every dispatch on `challenge.allowedActions` (its `allows` check), and a navigation drill permits only `select-cell`. The solver searched `EXTEND_*` regardless, so it could have proposed a route whose keys the live grid would swallow. The gate now sits in the search, on the **resolved action's kind** — never on the command, because `TOGGLE_FILTER` resolves to `filter-column` or `clear-filters` depending on the grid, and a static command-to-kind table would be wrong exactly where it matters. This can never hide a real route (the game accepts nothing outside `allowedActions`); collapsing those branches is the side effect, not the reason.

**Three prunes, all optimality-preserving, all needed to stay inside any sane budget.**
1. *Relevant kinds.* The search is restricted to `SPEC_KIND_ACTIONS[spec.kind]` — the map the eligibility gate already grades every variant against — plus `select-cell`, the prerequisite of everything, plus the selection kinds for `formatting`, because `set-format` writes to a selection. A sort or a filter reads the *focused column*, never the selection, so a range/row/column selection cannot shorten a sort-filter route.
2. *No navigating out of a range the search built.* A step that abandons an extended selection lands exactly where the same steps would have landed without ever extending, at the same depth, because `EXTEND_DOWN` moves the focus as far as `MOVE_DOWN`. **Never applied at the root**, which is subtle and cost real time: a composite's later part *starts* holding a range (formatting leaves both ends of the selection where they were), and pruning there told the solver that a player who has just bolded a row may never move again — whereupon it dutifully reported the challenge unsolvable.
3. *Whole-column formatting drops `select-range` entirely.* `Ctrl+Space` covers a column in one action from anywhere inside it, and an extended selection needs at least one action from the same cell to cover the same ground: extending can tie, never win. **Columns only** — `Shift+Space` takes the whole *grid* row, wider than the table, so `TOGGLE_BOLD` sees the blanks beyond the used range and *bolds* instead of unbolding, which is exactly why `formatting.unbold-header` has no solve without an extended selection that stops at the table's edge. One command is clipped to the used range and the other is not, so only one of them can stand in for building the range by hand.

**The solver found a live scoring exploit, and it is closed in the engine rather than hidden in the card.** `validateFormatting` only ever looked at the cells *inside* the required range, so **"make the Name values bold" was solvable by selecting the whole table and pressing Ctrl+B** — two actions, fewer than doing it properly, and it graded as a pass. With scoring keyed to action count that was the winning play, and the fastest-path card would have taught it to every player. Three options: report it honestly (and teach the exploit), hide it in the solver (and let the card lie about what is fastest), or close it. **Closed** (user's decision): `validateFormatting` now rejects a solve whose formatting spilled outside the columns/rows the target range touches, comparing against `challenge.initialGrid` so nobody is failed for a cell they never touched. The tolerance is narrow and deliberate — it is the line between `Ctrl+Space` (which catches only that column's own header) and `Ctrl+A` (which reaches across the sheet). This **changes grading**: a player who bolds extra columns now fails a formatting drill. It is the one deviation in this phase that changes what the game accepts, and it was taken deliberately, because the alternative is a trainer that rewards not doing the task.

**Ties are ranked by disturbance, because "shortest" is not the same as "worth teaching".** Equal-cost routes are ordered by how many cells they rewrite, gentlest first — measured from the engine (how many cells changed), never from the spec. Registry order breaks any remaining tie, so the result stays reproducible across runs and machines (§1a.5). The search collects the whole tie (up to 8) rather than the first `maxRoutes`, or the gentlest route could be thrown away before anything compared it.

**§10.1's "2 commands, not 3" for `gen.formatting.bold-column` is right about the mouse and wrong about the count.** The shipped note — click the first value, `Ctrl+Shift+Down`, `Ctrl+B` — is three actions, one of them a click. `Ctrl+Space` + `Ctrl+B` is two, *from inside the column*. Keyboard-only, reaching the column costs the moves it costs, so the solver's route runs to three or more at most difficulties. It is still strictly better than the note: keyboard-complete, and never longer. The test pins what is actually true — no `SELECT_TABLE` immediately before a bold, the route ends in `TOGGLE_BOLD`, it replays to a grid the real validator passes — rather than a count the plan asserted from a mouse-flavoured premise.
